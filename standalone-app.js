...
function copyRow(pick) {
  ...
  return [
    ...
    weekVal,                            // Column B: Week
    monthVal,                           // Column C: Month
    ticketIdVal,                        // Column D: Ticket ID
    agentVal,                           // Column E: Agent
...
function getReservedWorkbookCell(pick, colIdx) {
  if (!pick) return "";
  if (Array.isArray(pick)) return pick[colIdx] ?? "";
  if (Array.isArray(pick.rawValues) && pick.rawValues[colIdx] != null && pick.rawValues[colIdx] !== "") {
    return pick.rawValues[colIdx];
  }
  if (Array.isArray(pick.rawRow) && pick.rawRow[colIdx] != null && pick.rawRow[colIdx] !== "") {
    return pick.rawRow[colIdx];
  }
  return "";
}
...
function getWeekOfYearLabel(dateText) {
  ...
  if (!dateKey) return "";
  return String(computeQaWeekNumber(dateKey));
}
